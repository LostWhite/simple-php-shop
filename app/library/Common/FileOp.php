...
class FileOp 
{
   ...
   private  $handle;
   private  $filePath;
   function  __construct($fileOpenMethod_str='writeAndAdd',$fileName_str='')//路径，默认为空；模式，默认均为只读
   {
       //构造函数，完成数据成员的初始化
       $this->fileName_str=$fileName_str;
       $this->fileOpenMethod_str=$fileOpenMethod_str;
       $this->filePath = "C:/apache2.2/htdocs/bbs_new/public";
   }
   ...
   //$type order:订单  test:测试  
   public  function  OpenOrderFile($orderno , $tytpe ="order")
   {
	//   	echo  $_SERVER['DOCUMENT_ROOT'].$_SERVER['SCRIPT_FILENAME'];

   	 //$pathroot=    dirname(dirname( dirname(dirname(__FILE__)))); 
   	 $pathroot = $this->filePath;
   	 
       //析构函数
       if ( $tytpe =="order"){
       	 	$this->fileName_str= $pathroot."/fileorder/".$orderno.".html";
       }else{
       	$this->fileName_str= $pathroot."/filetest/".$orderno.".html";
       } 
  echo     	$this->fileName_str;
    $this->handle=  	$this->open();

   }
   
    public  function  WriteLineOrderFile($content)
   {
  		 return 	fputs ($this->handle,"<p>".$content."</p>");
   }
   
    public  function  CloseOrderFile()
   {
